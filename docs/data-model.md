...
interface Event {
  ...
  workspaceId: ULID;
  name: string;
  kind: string;                      // free text: 'Runway Show', 'Pop-Up' and so on
...
Rules evaluate on every assignment and surface inline: a marked seat, plus a violations panel. The drag still completes. Producers overrule rules constantly and the tool has to let them.
...
Current status = reduction over arrivals. Duplicate detection reads Dexie inside the write transaction, where two desks cannot race each other.
...
Generated by the deterministic simulator ([demo-data.md](./demo-data.md)). The widget layer does not know or care that the source is simulated. Swapping in a real feed later is a matter of writing one adapter.
...
interface FieldDef {
  ...
  order: number;
  archived?: boolean;                // archive, never hard-delete, so history survives
}
```
...
**Safe evaluation.** Formulas are parsed into an AST by a small Pratt parser (`lib/formula.ts`) over a whitelist: numbers, identifiers, `+ - * / ^ ( )`, comparisons, ternaries, and a fixed function set (`min`, `max`, `round`, `clamp`, `lookup`, `coalesce`, `log`). There is no `eval`, no `Function` constructor and no property access, so a user-authored formula cannot reach the DOM or the store. Unknown identifiers fail validation in the editor with the offending token highlighted, and an evaluation error degrades to `null` with a visible badge, leaving the row intact.
...
Both are one row per event. An advance and a settlement are each a single document several people edit over weeks, and keeping them whole means a party rename or a re-ordered price band is one write where it would otherwise be twenty. The cost is a read-modify-write on every change, which is the right trade at this size. Reading a workspace also passes its schema through `backfillSchema()`, so a record written before a vocabulary list existed still opens.
